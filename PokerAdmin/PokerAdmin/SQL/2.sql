...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[wntGetActivePlayPeriods]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[wntGetActivePlayPeriods]
GO
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[wntGetPlayersChips]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[wntGetPlayersChips]
GO
...
if @NoBots>0
	set @where = dbo.fnGetWhereCause(@where, 'u.IsBot=0')
...
if (@dtBegin is not null and @dtEnd is not null)
 ...
 set @Sql= 'SELECT TOP 1 dbo.fnNullAsMoney (dp.txAmount) as Amount, tgd.GameLogID as HandID
                   FROM Tx dp (nolock)  INNER JOIN TxGameDetail tgd  (nolock) ON dp.[id]=tgd.txid INNER JOIN UserAccount ua  (nolock) ON ua.[id]=dp.AccountID 
                   INNER JOIN vwUsers u  (nolock) ON u.[ID]=ua.UserID ' +
...
AS
   ...
   declare @CursOpened1  bit

   set @CursOpened=0
   set @CursOpened1=0

   DECLARE uReturn CURSOR FOR  SELECT  UserAccountID,Amount FROM  ReservedMoney (nolock) 
   ...
   BEGIN TRANSACTION wntClosePeriod
	...
	OPEN uReturn  -- Try return reserved money
		IF (@@ERROR <> 0) GOTO ErrorHandler
	 set @CursOpened1 = 1 

	FETCH NEXT FROM uReturn INTO @id, @Balance
		IF (@@ERROR <> 0) GOTO ErrorHandler
	    ...
	    BEGIN
                      if @Balance <>0
                        BEGIN
             		       set @Comment= 'Return reserved money for Play Period '+convert(varchar,@dFrom)+'-'+convert(varchar,@dTo)
	                   exec   [dbo].admSaveTransaction @Balance,@ID, @Comment,4
			IF (@@ERROR <> 0) GOTO ErrorHandler
                        END 
		      FETCH NEXT FROM uReturn INTO @id, @Balance
		IF (@@ERROR <> 0) GOTO ErrorHandler
	   ...
	   DEALLOCATE uReturn
	   set @CursOpened1=0
                UPDATE  ReservedMoney SET Amount=0


	OPEN uBalance  --- try clear balance
		IF (@@ERROR <> 0) GOTO ErrorHandler
	 set @CursOpened=1
	...
	FETCH NEXT FROM uBalance INTO @id, @Balance
	    ...
	    BEGIN
                    if @Balance <>0 
                     begin
	                    set @Balance= - @Balance
             		       set @Comment= 'Play Period Conclusion '+convert(varchar,@dFrom)+'-'+convert(varchar,@dTo)
...
if @NoBots>0
  begin
	  SELECT @tmpdtBg=MIN(cch.SessionStart) FROM ClientSessionHistory cch (nolock) INNER JOIN
                 vwUsers u (nolock) ON cch.UserID=u.[ID]  WHERE u.IsBot = 0
	  SELECT @tmpdtEND=MAX(cch.SessionStart) FROM ClientSessionHistory  cch (nolock) INNER JOIN
               vwUsers u (nolock) ON cch.UserID=u.[ID]  WHERE u.IsBot = 0
  end
else
  begin
 ...
 WHILE (@tmpdtEnd <= @dtEnd)
  BEGIN
if @NoBots>0
         SELECT @cnt=COUNT(*) FROM ClientSessionHistory cch (nolock) INNER JOIN 
                        vwUsers u (nolock)  ON cch.UserId = u.[ID] WHERE (cch.SessionStart BETWEEN @tmpdtBG AND @tmpdtEnd ) AND cch.UserID is not null 
                        AND u.IsBot=0 GROUP BY  UserID
else
...
CREATE PROCEDURE [dbo].wntGetActivePlayPeriods
 AS

 SELECT TOP 1 [ID],DateFrom,DateTo,IsClosed FROM PlayPeriods (nolock) WHERE isClosed=0 ORDER BY [ID]
GO
SET QUOTED_IDENTIFIER OFF 
GO
SET ANSI_NULLS ON 
...
CREATE PROCEDURE [dbo].wntGetPlayersChips
    @ID int 
 AS

declare @tb TABLE([ID] int  IDENTITY(1,1) PRIMARY KEY NONCLUSTERED ,PID int,LoginName varchar(25), Balance money, ThisID int)
declare @tID int

 INSERT INTO @tb SELECT u.[ID],u.LoginName, ua.Balance,0
                         FROM  dbo.vwUsers u (nolock)  INNER JOIN
                                     dbo.UserAccount ua (nolock) ON u.id = ua.UserID 
                         WHERE     (ua.CurrencyTypeID = 2) AND (u.UserTypeID = 1) 
                         ORDER BY ua.Balance DESC

  SELECT @tID = [ID] FROM @tb WHERE PID=@ID

  if @tID> 100
       DELETE @tb WHERE [ID] >99 AND [ID] <> @tID
  else
       DELETE @tb WHERE [ID] >100

  UPDATE @tb SET ThisID=1 WHERE [ID] = @tID

  SELECT [ID],LoginName,Balance,ThisID,PID FROM @tb ORDER BY [ID]
GO
SET QUOTED_IDENTIFIER OFF 
GO
SET ANSI_NULLS ON 
...
CREATE PROCEDURE [dbo].wntGetReportClosePeriod
 ...
 -- Fill table User's ID
    INSERT INTO @table ([PID],PlayPeriod) SELECT u.[ID] , @PlayPeriod as PL FROM vwUsers u WHERE UserTypeID=1 AND IsBot=0 ORDER BY u.[ID]
 ...
 -- determine MAX money won and Total momey won
    ...
    FROM (SELECT u.[id],MAX(tx.TxAmount) as HPoints, SUM(tx.TxAmount) as TotalPoints  FROM vwUsers u (nolock) INNER JOIN
                 UserAccount ua  (nolock) ON u.[ID]=ua.UserID INNER JOIN tx (nolock) on ua.[ID]=tx.AccountID WHERE ua.CurrencyTypeID=2  
                 AND u.UserTypeID=1 and tx.TxTypeID=6 and  u.IsBot=0
 ...
 -- determine count of minutes logged per day
    ...
    FROM (SELECT u.[id], (SUM(DATEDIFF ( minute , ch.SessionStart,ch.SessionEnd))/@Days) as MPDay  FROM vwUsers u (nolock) INNER JOIN
                 ClientSessionHistory ch  (nolock) ON u.[ID]=ch.UserID WHERE u.UserTypeID=1  and  u.IsBot=0
 ...
 -- determine Rank
  declare  @tb TABLE(tbRank int IDENTITY (1,1), [PrID] int DEFAULT 0 PRIMARY KEY, MWon money DEFAULT 0.00) 
    ...
    SELECT u.[id], SUM(tx.TxAmount) as MWon  FROM vwUsers u (nolock) INNER JOIN
                 UserAccount ua (nolock) ON u.[ID]=ua.UserID INNER JOIN tx (nolock) on ua.[ID]=tx.AccountID WHERE u.UserTypeID=1 and tx.TxTypeID=6 and 
                  (ua.CurrencyTypeID=2  or ua.CurrencyTypeID=1)  and  u.IsBot=0
...
/*    UPDATE @table SET HandsPlayed=hp.AllHands, HandsPerDay=hp.HdsPerDay
    FROM (SELECT u.[id],Count(*) as AllHands , (Count(*)/@Days) as HdsPerDay FROM Tx (nolock) INNER JOIN TxGameDetail tgd (nolock) ON 
     ...
     INNER JOIN vwUsers u (nolock) ON u.[id]=ua.userID WHERE (ua.CurrencyTypeID=2  or ua.CurrencyTypeID=1) AND tx.RecordDate BETWEEN @bPeriod AND @ePeriod 
     GROUP BY u.[id],tgd.GameLogID) hp  WHERE PID=hp.[id]*/

    UPDATE @table SET HandsPlayed=hp.AllHands, HandsPerDay=hp.HdsPerDay
    FROM (SELECT u.[id],Count(*) as AllHands , (Count(*)/@Days) as HdsPerDay FROM 
	  vwUsers u (nolock) INNER JOIN  UserAccount ua (nolock) ON u.[id]=ua.userID INNER JOIN  tx (nolock) ON tx.AccountID=ua.[id]
                WHERE (ua.CurrencyTypeID=2  or ua.CurrencyTypeID=1)  and  u.IsBot=0  AND 
                tx.RecordDate BETWEEN @bPeriod AND @ePeriod  AND (tx.TxTypeID=5 OR tx.TxTypeID=6)
                GROUP BY u.[id]) hp  WHERE PID=hp.[id]

/*
SELECT u.[id],Count(*) as AllHands , (Count(*)/@Days) as HdsPerDay FROM Tx (nolock) INNER JOIN UserAccount ua (nolock) ON ua.[id]=tx.AccountID
     INNER JOIN vwUsers u (nolock) ON u.[id]=ua.userID WHERE (ua.CurrencyTypeID=2  or ua.CurrencyTypeID=1) AND tx.RecordDate BETWEEN @bPeriod AND @ePeriod 
     AND (tx.TxTypeID=5 OR tx.TxTypeID=6)
     GROUP BY u.[id]) hp  WHERE PID=hp.[id]
*/
 ...
 -- Practice Hands
    ...
    FROM (SELECT u.[id],Count(*) as AllHands  FROM vwUsers u (nolock) INNER JOIN UserAccount ua (nolock) ON u.[id]=ua.userID 
    INNER JOIN Tx (nolock) ON ua.[id]=tx.AccountID 
    WHERE ua.CurrencyTypeID=1 AND tx.RecordDate BETWEEN @bPeriod AND @ePeriod 
     AND (tx.TxTypeID=5 OR tx.TxTypeID=6)  and  u.IsBot=0
     GROUP BY u.[id]) hp  WHERE PID=hp.[id]
...
/*    UPDATE @table SET HandsWon=hp.AllHands
    ...
    UPDATE @table SET HandsWon=hp.AllHands
    FROM (SELECT u.[id],Count(*) as AllHands  FROM vwUsers u (nolock) INNER JOIN UserAccount ua (nolock) ON u.[id]=ua.userID
    INNER JOIN tx (nolock) ON ua.[id]=tx.AccountID WHERE (ua.CurrencyTypeID=2  or ua.CurrencyTypeID=1) AND tx.RecordDate BETWEEN @bPeriod AND @ePeriod 
     AND tx.TxTypeID=6  and  u.IsBot=0 GROUP BY u.[id]) hp  WHERE PID=hp.[id]
  ...
  begin
	set @where = dbo.fnGetWhereCause(@where, 'u.[ID]=' + convert(varchar,@UserID))
	 set @sql='SELECT dbo.GetDatePart(tx.RecordDate) as HandDate,tgd.GameLogID as HandID,gp.[Name] as game,u.[id] as UserID,u.LoginName,Count(*) as AllHands  FROM Tx 
		      (nolock) INNER JOIN TxGameDetail tgd  (nolock) ON 
		      tx.[id]=tgd.txid INNER JOIN UserAccount ua  (nolock) ON ua.[id]=tx.AccountID
		     INNER JOIN vwUsers u  (nolock) ON u.[id]=ua.userID INNER JOIN GameProcess gp  (nolock) ON gp.ID=tgd.GameProcessID '+ @where +
		    ' GROUP BY dbo.GetDatePart(tx.RecordDate),tgd.GameLogID,gp.[Name],u.[id],u.LoginName'
  end
else
	 set @sql='SELECT dbo.GetDatePart(tx.RecordDate) as HandDate,0 as HandID,"" as game,0 as UserID,"" as LoginName,Count(*) as AllHands  FROM Tx  (nolock)
		      INNER JOIN TxGameDetail tgd  (nolock) ON 
		      tx.[id]=tgd.txid INNER JOIN UserAccount ua  (nolock) ON ua.[id]=tx.AccountID
		      INNER JOIN GameProcess gp  (nolock) ON gp.ID=tgd.GameProcessID '+ @where +
...
if (@UserID>0)
	...
	if @UserID>0
                 set @sql=@sql+'INNER JOIN vwUsers u  (nolock) ON u.[id]=ua.userID '

                 set @sql=@sql+ 'INNER JOIN CurrencyType ct  (nolock) ON ct.ID=ua.CurrencyTypeID '+ @where +' GROUP BY '
...
set @sql='SELECT u.id,u.LoginName, CONVERT(money, COUNT(tx.id)) / '+convert(varchar,@koef)+' AS Rate 
                FROM         dbo.vwUsers u (nolock) INNER JOIN
                      dbo.UserAccount ua  (nolock) ON ua.UserID = u.id INNER JOIN
                      dbo.Tx  (nolock) ON ua.id = dbo.Tx.AccountID '+ @where+' 
...
set @sql='SELECT gp.Name, CONVERT(money, COUNT(u.id)) / '+convert(varchar,@koef)+' AS Rate, gp.id
                FROM         dbo.vwUsers u (nolock) INNER JOIN
                      dbo.UserAccount ua (nolock) ON ua.UserID = u.id INNER JOIN
                      dbo.Tx (nolock) ON ua.id = dbo.Tx.AccountID INNER JOIN
                      dbo.TxGameDetail tgd (nolock) ON tgd.TxID = dbo.Tx.id INNER JOIN
                      dbo.GameProcess gp (nolock) ON tgd.GameProcessID = gp.id '+ @where+' 
...
if @CurrencyTypeID<=0 
 ...
 set @Sql= 'SELECT p.Amount,p.UserID,p.LoginName,MAX(tx.RecordDate)  as DateWon FROM
                   ...
                   FROM Tx dp  (nolock) INNER JOIN UserAccount ua (nolock) ON ua.[id]=dp.AccountID INNER JOIN vwUsers u (nolock) ON ua.UserID=u.[ID] ' +
...
set @sql='SELECT     id, LoginName, RecordDate FROM dbo.vwUsers u (nolock) 
                 WHERE     (id NOT IN (SELECT  u.[ID]  FROM  dbo.TxGameDetail tgd  (nolock), dbo.Tx (nolock), dbo.vwUsers u (nolock) INNER JOIN
 ...
 AS
  ...
  if @ID<=0  
	...
	set @ID = scope_identity()
            ...
            UPDATE  PlayPeriods SET DateFrom=@DateFrom,DateTo=@DateTo WHERE [ID]=@ID
...
if @UserID > 0
       set @sql='SELECT u.[id] as UserID,u.LoginName,SUM(DATEDIFF(mi,cs.SessionStart,cs.SessionEnd)) as SpentTime  FROM ClientSessionHistory cs (nolock) 
                 INNER JOIN vwUsers u (nolock) ON u.[id]=cs.UserID '+ @where +
...
if @UserID>0
 ...
 set @Sql= 'SELECT  dp.RecordDate,u.LoginName,u.[ID] ,tt.Name,SUM(dbo.fnNullAsMoney (dp.txAmount)) as Amount
                   FROM Tx dp  (nolock) INNER JOIN UserAccount ua (nolock) ON ua.[ID]=dp.AccountID INNER JOIN [User] u (nolock) ON u.[ID]=ua.UserID
                   INNER JOIN txType tt (nolock) ON dp.TxTypeID=tt.[ID]'+
...
if (@dtBegin is not null and @dtEnd is not null)
  ...
  set @sql='SELECT SUM(t.Registered) as Registered, SUM(t.Played) as Played FROM
                  (SELECT COUNT(*) as Registered, 0 as Played FROM  vwUsers u (nolock) '+ @where + '
                  UNION ALL
                  SELECT 0 as Registered,COUNT(u.[id]) as Played FROM vwUsers u  (nolock)
                  WHERE u.[ID] IN (SELECT u.[ID] FROM vwUsers u  (nolock) INNER JOIN UserAccount ua  (nolock) ON ua.UserID=u.[ID] 
                  INNER JOIN tx (nolock) ON tx.AccountID=ua.[ID] '+ @where + ' GROUP BY u.[ID] )) as t'
   print @sql
  exec(@sql)
GO
SET QUOTED_IDENTIFIER OFF 
