...
;---Writing Settings Section
  WriteRegStr HKCU "SOFTWARE\BikiniPoker\Settings" "host" "212.1.89.72"
  WriteRegStr HKCU "SOFTWARE\BikiniPoker\Settings" "port" "4000"
...
;---Writing Uninstall Section
  ...
  CreateShortCut "$DESKTOP\BikiniPoker.lnk" "$INSTDIR\BikiniPoker.exe"

; installs the start menu shortcuts
  CreateDirectory "$SMPROGRAMS\BikiniPoker"
  CreateShortCut "$SMPROGRAMS\BikiniPoker\BikiniPoker.lnk" "$INSTDIR\BikiniPoker.exe"
...
Delete "$INSTDIR\files\Avatar.swf"
Delete "$INSTDIR\files\SGW.swf"
Delete "$INSTDIR\files\Track1.mp3"
Delete "$INSTDIR\files\Track2.mp3"
Delete "$INSTDIR\files\WinstonPoker.swf"
