...
namespace Admin.Misc
{
	...
	/// </summary>
	public class BonusRules : Common.Web.Page
	{
		...
		{
			DataTable oDT = DBase.GetDataTable(Config.DbGetBonusRules);
			foreach(DataRow oDR in oDT.Rows)
			{
				...
				oTB.ID = string.Format("MinValueForBonus_{0}", nID);
				oTB.Text = oDR["MinValueForBonus"].ToString();
				...
				oTB.MaxLength = 30;
				oTB.Width=Unit.Percentage(100);
				oTB.ID = string.Format("BonusValue_{0}", nID);
				oTB.Text = oDR["BonusValue"].ToString();
				oTC.Controls.Add(oTB);
				oTR.Cells.Add(oTC);
		...
		public void btnSave_click(object sender, System.EventArgs e)
		{
			...
			DBase.BeginTransaction("admWebSaveBonusRules");
			DataTable oDT = DBase.GetDataTableBatch(Config.DbUpdateBonusRules);
			if ( oDT != null )
			{
				...
				{
					nID =GetID(sKey,"MinValueForBonus_");
					if ( nID > 0 )   SetValue(oDV, nID,"MinValueForBonus",sKey);
					nID =GetID(sKey,"BonusValue_");
					if ( nID > 0 )   SetValue(oDV, nID,"BonusValue",sKey);
			...
			if ( bRes )
			{
				...
				DBase.CommitTransaction("admWebSaveBonusRules");
				BindData();
			...
			else
			{
				...
				DBase.RollbackTransaction("admWebSaveBonusRules");
