...
type
  ...
  public
    ...
    //
    constructor Create(aTable: TBotTable; aUser: TBotUser; aCardPack: TCardList = nil);
...
constructor TBotAutoResponse.Create(aTable: TBotTable; aUser: TBotUser; aCardPack: TCardList);
var
...
begin
  Result := nil;
  if (FAvailableAnswers.Count <= 0) then Exit;
{  if (FTable.StakeType <> BST_FIX_LIMIT) and (FUser.CountOfRases > 1) then begin
    Result := GetMidleAnswerResult;
    Exit;
  end;}
  Result := FAvailableAnswers.Items[FAvailableAnswers.Count-1];
  if (Result.AnswerType = ACT_RAISE) then
    FUser.CountOfRases := FUser.CountOfRases + 1;
end;
