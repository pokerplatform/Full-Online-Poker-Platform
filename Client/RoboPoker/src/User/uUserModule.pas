...
const
  UserPasswordMagicWord = 'P0ker0a5i5uSerPa5sworD';
...
type
  ...
  private
    ...
    FUserEMail: String;
    FUserEMailAlerts: Boolean;
    FUserBuddyAlerts: Boolean;
    FUserSexID: Integer;
    ...
    FUserQualification: String;
    FUserQOMPID: Integer;
    FUserBonusAccount: Currency;
  ...
  public
    ...
    property  UserEMailAlerts: Boolean read FUserEMailAlerts;
    property  UserBuddyAlerts: Boolean read FUserBuddyAlerts;
    ...
    property  UserQualification: String read FUserQualification;
    property  UserQOMPID: Integer read FUserQOMPID;
    property  UserBonusAccount: Currency read FUserBonusAccount;
    ...
    procedure PopupMessagesHistory;
    ...
    procedure Do_UpdateProfile(FirstName, LastName, Location: String;
      ShowLocation, EMailAlerts, BuddyAlerts: Boolean; SexID: Integer);
...
begin
  ...
  FPlayerLogoFileName := PlayerLogoDefaultFileName;
  FPlayerLogoID := -1;
...
begin
  Clear;
  FPopupMessages.Free;
  if not (FChangePlayerLogoForm <> nil) then
...
begin
  ...
  begin
    ...
    ParserModule.Send_GetBalanceInfo(FUserID);
    ParserModule.Send_GetMailingAddress(FUserID);
...
begin
  ...
  ParserModule.Send_UpdateProfile(FUserID, FirstName, LastName,
    Location, ShowLocation, EMailAlerts, BuddyAlerts, AvatarID, SexID);
end;
...
begin
  ...
  begin
    ...
    FUserEMailAlerts := XMLRoot.Attributes['emailalerts'] = '1';
    FUserBuddyAlerts := XMLRoot.Attributes['buddyalerts'] = '1';
    FUserLocation := XMLRoot.Attributes['location'];
    FUserShowLocation := XMLRoot.Attributes['showlocation'] = '1';
    FUserQualification := XMLRoot.Attributes['userqualification'];
    FUserQOMPID := strtointdef(XMLRoot.Attributes['qompid'], 0);
    FUserBonusAccount := Conversions.Str2Cur(XMLRoot.Attributes['bonusaccount']);
...
begin
  ...
  if NewPlayerLogoID <> FPlayerLogoID then
    ParserModule.Send_UpdateProfile(FUserID, FUserFirstName, FUserLastName,
      FUserLocation, FUserShowLocation, FUserEMailAlerts, FUserBuddyAlerts,
...
procedure TUserModule.PopupMessagesHistory;
begin
  if Logged then
  begin
    FOnActionLogin := nil;
    ParserModule.Send_PopupMessagesHistory(FUserID);
  end
  else
  begin
    FOnActionLogin := PopupMessagesHistory;
    Login;
  end;
end;
