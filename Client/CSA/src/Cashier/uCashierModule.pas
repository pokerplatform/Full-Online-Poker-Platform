...
begin
  ...
  FCurrentDateTo := now;
  ParserModule.Send_GetTransactionHistory(RealMoneyCurrencyID,
    FCurrentTopLast, FCurrentDateFrom, FCurrentDateTo);
  ParserModule.Send_GetTransactionHistory(BonusCurrencyID,
    FCurrentTopLast, FCurrentDateFrom, FCurrentDateTo);
...
begin
  ...
  begin
    ...
    UserModule.Do_GetMoreChips;

    if UserModule.NeedGetMoreChipsRequest then //to notify user about raising his account
    begin
      UserModule.NeedGetMoreChipsRequest := false;
      ThemeEngineModule.ShowMessage(cstrUserGetMoreChips);
    end;
  end;
...
begin
  ParserModule.Send_ChangeMailingAddress(FirstName, LastName,
...
begin
  ParserModule.Send_Withdrawal(RealMoneyCurrencyID, Amount);
end;
...
begin
  ...
  CurrencyID := strtointdef(XMLRoot.Attributes['currencyid'], 0);
  if CurrencyID > 0 then
    FHistory.Add(CurrencyID).LoadFromXML(XMLRoot, True);
