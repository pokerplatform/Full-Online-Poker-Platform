...
type
  TTournamentLobbyForm = class(TForm)
    ...
    TournamentLobbyButtonsBackgroundPanel: TTeHeaderPanel;
    UnRegisterButton: TTeButton;
    TournamentInfoButton: TTeButton;
    AddonButton: TTeButton;
    AutoRebuyCheckBox: TTeCheckBox;
    RebuyTimer: TTimer;
    ...
    procedure AddonButtonClick(Sender: TObject);
    procedure AutoRebuyCheckBoxClick(Sender: TObject);
    procedure RebuyTimerTimer(Sender: TObject);
  ...
  public
    ...
    procedure StartWork(TournamentName: String; Logged, Registered: Boolean; TournamentType,
      TournamentState: Integer; BuyIn: Currency = 0; RebuyCount: Integer = 0;
      RebuyAllowed: Boolean = false; AutoRebuy: Boolean = false;AddonAllowed: Boolean = false);
    procedure UpdateState(TournamentName : String; Logged, Registered: Boolean;
      TournamentType, TournamentState: Integer; RebuyAllowed: Boolean; AutoRebuy: Boolean; AddonAllowed: Boolean);
...
uses
  ...
  uTournamentModule,
  uConversions,
  uConstants, uUserModule, Math, uProcessModule;
...
begin
  {hemeEngineModule.FormsChangeConstraints(Self,
...
procedure TTournamentLobbyForm.StartWork(TournamentName: String; Logged, Registered: Boolean; TournamentType,
      TournamentState: Integer; BuyIn: Currency = 0; RebuyCount: Integer = 0;
      RebuyAllowed: Boolean = false; AutoRebuy: Boolean = false;AddonAllowed: Boolean = false);
begin
  FBuyIn := BuyIn;
  FRebuyCount := RebuyCount;
  Caption := TournamentName + ' Lobby';
  //TeForm.Caption := Caption;
  UpdateState(TournamentName, Logged, Registered, TournamentType, TournamentState, RebuyAllowed,AutoRebuy,AddonAllowed);
...
begin
  ...
  begin
    ...
    if (RebuyAllowed or AddonAllowed) and FRegistered then
    begin
      if RebuyAllowed then
      begin
       AddonButton.Caption := 'Rebuy';
       AddonButton.Visible := true;
       AutoRebuyCheckBox.Visible := true;
       AutoRebuyCheckBox.Checked := AutoRebuy;
      end
      else
      begin
       AddonButton.Caption := 'Add - On';
       AutoRebuyCheckBox.Visible := false;
      end;
    end
    else
    begin
...
procedure TTournamentLobbyForm.AddonButtonClick(Sender: TObject);
begin
  TournamentModule.Do_Addon(ID,1);
  AddonButton.Enabled := false;
end;

procedure TTournamentLobbyForm.AutoRebuyCheckBoxClick(Sender: TObject);
var
 Value: Integer;
begin
  Value := 0;
  if AutoRebuyCheckBox.Checked then
   Value := 1;

  TournamentModule.Do_AutoRebuy(ID,Value);
  AutoRebuyCheckBox.Enabled := false;
  RebuyTimer.Enabled := true;
  ProcessModule.UpdateOptions;
end;

procedure TTournamentLobbyForm.RebuyTimerTimer(Sender: TObject);
begin
  AutoRebuyCheckBox.Enabled := true;
  RebuyTimer.Enabled := false;
end;

procedure TTournamentLobbyForm.OkButtonClick(Sender: TObject);
begin
 TournamentModule.Do_Addon(ID,1);
end;

procedure TTournamentLobbyForm.EnableAddonButton;
begin
  AddonButton.Enabled := true;
end;
